...
def calculate_strong_divergence_distance_matrix(voting_matrix):
    """Calcula a matriz de distâncias de divergência forte entre deputados."""
    n = voting_matrix.shape[0]
    distance_matrix = np.zeros((n, n))

    for i in range(n):
        for j in range(i + 1, n):
            # Apenas comparações onde ambos votaram Sim (1) ou Não (-1), ignorando abstenções (0)
            ...
            if total_valid == 0:
                ...
                differences = np.sum(voting_matrix[i, mask] != voting_matrix[j, mask])  # Conta Sim ≠ Não
...
for period_start, period_end in periods:
    ...
    mapping_votes = {'Não': -1, 'Sim': 1, 'Artigo 17': np.nan, 'Abstenção': np.nan, 'Obstrução': -1}
    ...
    original_distances = calculate_strong_divergence_distance_matrix(df_filled.to_numpy())
    ...
    #    mds_pairwise_distances = pairwise_distances(mds_distances, metric='hamming')
...
df_deputy_distances.to_csv(os.path.join(processed_path, f"deputy_distances_forte.csv"), index=False)

# Converter resultados gerais em DataFrame
df_results = pd.DataFrame(results)

# Salvar resultados gerais em CSV
df_results.to_csv(os.path.join(processed_path, f"average_mds_distances_forte.csv"), index=False)

# Converter os resultados de stress em um DataFrame
df_stress = pd.DataFrame(stress_results)

# Salvar o DataFrame de stress em um arquivo CSV
df_stress.to_csv(os.path.join(processed_path, f"average_mds_distances_forte.csv"), index=False)
...
df_partido_distancias.to_csv(os.path.join(processed_path, f"partido_distancias_forte.csv"), index=False)
...
fig, axes = plt.subplots(6, 2, figsize=(6, 20))

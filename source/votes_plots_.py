...
cluster_name = 'kmeans' if metric == 'euclidean' else 'kmedoids' #also can use 'sphericalkmeans' or 'nmf'
...
def calculate_average_euclidean_distance(points, metric):
...
df_votos_ = df_votos.copy()
df_votos_['month'] = df_votos_['data'].dt.strftime('%Y%m')
df_votos_abs = df_votos_[df_votos_['voto'].isin(['Abstenção'])]

df_votos_ = df_votos_['month'].sort_values().value_counts().reset_index()
df_votos_.columns = ['month','total']
df_votos_['data'] = pd.to_datetime(df_votos_['month'].astype(str)+'01',format = '%Y%m%d')
df_votos_ = df_votos_.sort_values('data')


df_votos_abs_ = df_votos_abs['month'].sort_values().value_counts().reset_index()
df_votos_abs_.columns = ['month','abs']

df_votos_ = df_votos_.merge(df_votos_abs_, how = 'left', on = 'month').fillna(0)
df_votos_['pct'] = df_votos_['abs']/df_votos_['total']

# Criar a figura e o eixo principal
fig, ax1 = plt.subplots(figsize=(12, 6))

# Configurar o primeiro eixo Y (para "total")
color = 'tab:blue'
ax1.set_xlabel('Data')
ax1.set_ylabel('Total', color=color)
ax1.plot(df_votos_['data'], df_votos_['total'], color=color, label='Total')
ax1.tick_params(axis='y', labelcolor=color)

# Criar o segundo eixo Y (para "abs")
ax2 = ax1.twinx()  
color = 'tab:red'
ax2.set_ylabel('Abs', color=color)
ax2.plot(df_votos_['data'], df_votos_['abs'], color=color, linestyle='dashed', label='Abs')
ax2.tick_params(axis='y', labelcolor=color)

# Melhorando a formatação do eixo X
plt.xticks(rotation=45)
plt.title('Evolução de Total e Abs ao Longo do Tempo')
...
# Plotar "total" no primeiro subplot
ax1.plot(df_votos_['data'], df_votos_['total'], label='Total')
ax1.set_ylabel('Total')
ax1.set_title('Evolução de Total e Abs ao Longo do Tempo')
ax1.legend()
ax1.grid(True)

# Plotar "abs" no segundo subplot
ax2.plot(df_votos_['data'], df_votos_['abs'],  label='Abs')
ax2.set_ylabel('Abs')
# ax2.set_xlabel('Data')
ax2.legend()
ax2.grid(True)
...
# Plotar "abs" no segundo subplot
ax3.plot(df_votos_['data'], df_votos_['pct'],  label='Abs')
ax3.set_ylabel('pct')
ax3.set_xlabel('Data')
ax3.legend()
ax3.grid(True)
...
end_month = pd.Timestamp('2023-09-01')
...
for period_start, period_end in periods:
    ...
    df_votos_f = df_votos_f[df_votos_f['voto'].isin(['Sim','Não'])]
    ...
    df_dep_size = df_votos_f[df_votos_f['idVotacao'].isin(list_vote_id)][['idDeputado', 'nome']].value_counts().reset_index()
    df_dep_size.columns = ['idDeputado', 'nome','size']    
    df_dep_size['pct'] = df_dep_size['size'] / len(list_vote_id)
    list_dep_id = df_dep_size[df_dep_size['pct'] > pct_dep]['idDeputado']

    print(f"  - tamanho de votacoes: {list_vote_id.shape[0]}")
    print(f"  - tamanho de deputados:{list_dep_id.shape[0]}")


    df_votos_f = df_votos_f[
        (df_votos_f['idVotacao'].isin(list_vote_id)) & 
    ...
    df_na = df_pivot.isna().sum()/len(df_pivot)
    df_na = df_na.reset_index().rename(columns = {0:'pct'})
    df_na.pct.hist()
    df_tmp = df_na.pct.describe().to_frame().T[['count', 'mean', 'std','50%']]
    df_tmp['start']= period_start
    df_tmp['end']= period_end
    
    df_tmp['votes']= list_vote_id.shape[0] 
    df_tmp['deps']= list_dep_id.shape[0] 
    ...
    original_distances = pairwise_distances(df_filled.to_numpy(), metric='hamming')
    ...
    if cluster_name == 'kmeans':# K-means clustering com k=2 e k=3        
        cluster2 = KMeans(n_clusters=2, random_state=42)        
        cluster2.fit(positions)
        clusters_k2 = cluster2.fit_predict(positions) 

        cluster3 = KMeans(n_clusters=3, random_state=42)
        cluster3.fit(positions)
        clusters_k3 = cluster3.fit_predict(positions)

        # Calcular os centróides de cada cluster
        centroides_k2 = cluster2.cluster_centers_
        ...
        # avg_euclidean_distance = calculate_average_euclidean_distance(positions, 'metric')
        avg_euclidean_distance = calculate_average_euclidean_distance(positions, 'euclidean')

        
    elif cluster_name == 'kmedoids': # K-Medoids clustering com k=2 e k=3 usando distância de Hamming        
        cluster2 = KMedoids(n_clusters=2, metric='precomputed', random_state=42)
        cluster2.fit(original_distances)
        clusters_k2 = cluster2.fit_predict(original_distances) 
        
        cluster3 = KMedoids(n_clusters=3, metric='precomputed', random_state=42)
        cluster3.fit(original_distances)
        clusters_k3 = cluster3.fit_predict(original_distances)
        
        # Obtenha os medoids (índices dos pontos que são os medoids)
        medoids_k2 = cluster2.medoid_indices_

        # 1. Calcular a distância entre os medoids
        distancia_entre_centroides_k2 = pairwise_distances(df_filled.to_numpy()[medoids_k2])[0, 1]
        # print(f"Distância entre os medoids (k=2): {distancia_entre_medoids_k2}")
        
        # 2. Calcular a distância de cada deputado ao medoid do seu cluster
        clusters_k2 = cluster2.labels_
        
        distancias_deputados_medoids = []
        for i, cluster in enumerate(clusters_k2):
            distancia = pairwise_distances([df_filled.to_numpy()[i]], [df_filled.to_numpy()[medoids_k2[cluster]]])[0, 0]
            distancias_deputados_medoids.append(distancia)
        
        distancias_deputados_medoids = np.array(distancias_deputados_medoids)
        # print(f"Distâncias de cada deputado ao seu medoid: {distancias_deputados_medoids}")
        # 
        # 3. Distância média
        avg_euclidean_distance = np.mean(distancias_deputados_medoids)
        # print(f"Distância euclidiana média dos deputados ao medoid do cluster: {avg_euclidean_distance}")

    avg_sqeuclidean_distance = calculate_average_sqeuclidean_distance(positions)
    # elif cluster == 'sphericalkmeans': # K-Medoids clustering com k=2 e k=3 usando distância de Hamming        
    #     cluster2 = SphericalKMeans(n_clusters=2, metric='precomputed', random_state=42)
    #     cluster3 = SphericalKMeans(n_clusters=2, metric='precomputed', random_state=42)
    #     positions_ = original_distances
    ...
    distancias_EP_df.to_csv(os.path.join(processed_path,f"distanciasEP_{period_start.strftime('%Y-%m')}_{metric}.csv"))
    ...
    distance_matrix = squareform(pdist(positions, metric=metric))
   ...
   # df_filled['normalized_avg_distance'] = df_filled['avg_distance'] / mean_avg_distance
    ...
    df_filled['kmeans_2'] = cluster2.labels_
    df_filled['kmeans_3'] = cluster2.labels_
    ...
    if cluster_name == 'kmeans':# K-means clustering com k=2 e k=3        
        ...
        # Obter os medoids em termos de posições (no espaço MDS ou outro)
        medoids_positions_k2 = positions[medoids_k2]
        
        # Distância de cada deputado para o medoid do próprio cluster
        df_filled_reset['DistCentrProprioCluster'] = np.where(
            clusters_k2 == 0, 
            pairwise_distances(positions, medoids_positions_k2[0].reshape(1, -1)).flatten(), 
            pairwise_distances(positions, medoids_positions_k2[1].reshape(1, -1)).flatten()
        )
        
        # Distância de cada deputado para o medoid do outro cluster
        df_filled_reset['DistCentrOutroCluster'] = np.where(
            clusters_k2 == 0, 
            pairwise_distances(positions, medoids_positions_k2[1].reshape(1, -1)).flatten(), 
            pairwise_distances(positions, medoids_positions_k2[0].reshape(1, -1)).flatten()
...
# Plotar cada variável em um subplot
axes[0].plot(df_agg['start'], df_agg['mean'], label='Mean', color='tab:blue')
axes[0].plot(df_agg['start'], df_agg['50%'], label='median', color='tab:orange')
axes[0].set_ylabel('missing values (%)')
axes[0].legend()
axes[0].set_title(f'{pct_dep}%')

axes[0].grid(True)

axes[2].plot(df_agg['start'], df_agg['deps'], label='Deps', color='tab:green')
axes[2].set_ylabel('Deps')
axes[2].legend()
axes[2].grid(True)

axes[1].plot(df_agg['start'], df_agg['votes'], label='Votes', color='tab:red')
...
df_deputy_distances.to_csv(os.path.join(processed_path,f"deputy_distances_{metric}_v2.csv"), index=False)
# df_deputy_distances = pd.read_csv(os.path.join(processed_path,f"deputy_distances_{metric}"))
...
df_results.to_csv(os.path.join(processed_path,f"average_mds_distances_{metric}_v2.csv"), index=False)
# df_results= pd.read_csv(os.path.join(processed_path,f"average_mds_distances_{metric}"))
...
# Customizações
plt.title('Evolução Temporal da Polarização na Câmara Federal')
plt.xlabel('Semestre/Ano')
plt.ylabel('Distância Média entre Deputados nas Votações Nominais')
plt.legend()
...
df_partido_distancias.to_csv(os.path.join(processed_path,f"partido_distancias_{metric}_v2.csv"), index=False)
df_partido_distancias = pd.read_csv(os.path.join(processed_path,f"partido_distancias_{metric}"))
...
plt.title('Evolução das Distâncias Normalizadas Médias - Partidos Selecionados')
plt.xlabel('Período')
plt.ylabel('Média das Distâncias Normalizadas')
plt.grid(False)
...
plt.legend(title='Partidos', loc='center left', bbox_to_anchor=(1, 0.5), fontsize='small')
...
selected_periods = ['2014-01', '2015-01', '2016-01', '2017-01', '2018-01', '2019-01', '2020-01', '2021-01', '2022-01', '2023-01']
...
titles = {
    (plot_dates[0], 0): "Distância dos Deputados para Governista - 2021",
    (plot_dates[0], 1): "Distância dos Deputados para Oposicionista - 2021",
    (plot_dates[1], 0): "Distância dos Deputados para Governista - 2023",
    (plot_dates[1], 1): "Distância dos Deputados para Oposicionista - 2023",
}
...
for period in plot_dates:
    for k in clusters:
        ...
        ax.set_xlabel('Distância Normalizada ao Próprio Centróide')
        ax.set_ylabel('Distância Normalizada ao Outro Centróide')
...
fig.suptitle('Evolução Anual da Distância entre Deputados', fontsize=16)
